...
TEST_WRAPPER = 'fuel_env'  # env wrapper used for testing model performance
SALVAGE_WRAPPER = 'no_fuel_env'  # env wrapper used for retraining subtracted model

POLICY = 'MlpPolicy'

FIRST_TRAIN_TIMESTEPS = 250000
SECOND_TRAIN_TIMESTEPS = 100000
SALVAGE_TIMESTEPS = 100000

metadata = {
    'BASE_ENV': BASE_ENV,
    'FIRST_WRAPPER': FIRST_WRAPPER,
    'SECOND_WRAPPER': SECOND_WRAPPER,
    'TEST_ENV': TEST_WRAPPER,
    'SALVAGE_WRAPPER': SALVAGE_WRAPPER,
    'POLICY': POLICY,
    'FIRST_TRAIN_TIMESTEPS': FIRST_TRAIN_TIMESTEPS,
    'SECOND_TRAIN_TIMESTEPS': SECOND_TRAIN_TIMESTEPS,
    'SALVAGE_TIMESTEPS': SALVAGE_TIMESTEPS
}

rewards_dict = {
    'init': { 'total_reward': [], 'fuel_reward': [] },
    'pre_update': { 'total_reward': [], 'fuel_reward': [] },
    'post_update': { 'total_reward': [], 'fuel_reward': [] },
    'sub_update': { 'total_reward': [], 'fuel_reward': [] },
    'salvage': { 'total_reward': [], 'fuel_reward': [] }
}
...
TEST_ENV = wrappers[TEST_WRAPPER]
SALVAGE_ENV = wrappers[SALVAGE_WRAPPER]
...
utils.test_model(TEST_ENV, model, rewards_dict, 'init', render=False, num_episodes=20000)
...
utils.test_model(TEST_ENV, model, rewards_dict, 'pre_update', render=False, num_episodes=20000)
...
utils.test_model(TEST_ENV, model, rewards_dict, 'post_update', render=False, num_episodes=20000)
...
utils.test_model(TEST_ENV, model, rewards_dict, 'sub_update', render=False, num_episodes=20000)

# TESTING ADDING THE FOLLOWING LINES UNTIL utils.save_to_json()
...
model.set_env(SALVAGE_ENV)
model.learn(total_timesteps=SALVAGE_TIMESTEPS)

utils.test_model(TEST_ENV, model, rewards_dict, 'salvage', render=False, num_episodes=20000)
